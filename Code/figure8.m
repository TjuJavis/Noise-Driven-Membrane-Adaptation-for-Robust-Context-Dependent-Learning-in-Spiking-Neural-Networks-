...
for i = 1:3     %开始循环
    ...
    [PerCorrect, ~, ~, ~, ~, ~] = ...
        spikingNetworkContextLearning(nTrial, simd(i), tau, Am, 1);
    ...
    [PerCorrect, FiringRate, FIndex, W12perTrial, W23perTrial, RasterPlot] = ...
        spikingNetworkContextLearning(nTrial, simd(i), tau, Am, 2);
    Overallpercentcorrect2(i) = sum(PerCorrect) / nTrial * 100;
    overalPerCorrect2(i,:)    = PerCorrect;
    % 打印正确试验的百分率。
    fprintf('Overall percent correct trials: %2.2f.\n', Overallpercentcorrect2(i));
end
...
for i=1:3
    ...
    overalPerCorrect2(i,:) = imfilter(double(overalPerCorrect2(i,:)), TrialWindow, 'same', 0) * 100; % 使用平滑窗口过滤数据进行平滑
    ToveralPerCorrect2(i,:) = overalPerCorrect2(i,TrialIndex); % 获取该窗口内的数据作为绘图数据
end
% 创建图像窗口
figure('Name', 'Figure 8', 'Units', 'Inches', 'Position', [0, 0, 8, 6], 'NumberTitle', 'off');
...
plot(TrialIndex, ToveralPerCorrect(3,:), '-y', 'LineWidth', LINE_WIDTH);
plot(TrialIndex, ToveralPerCorrect2(1,:), '--m', 'LineWidth', LINE_WIDTH);
plot(TrialIndex, ToveralPerCorrect2(2,:), '--c', 'LineWidth', LINE_WIDTH);
plot(TrialIndex, ToveralPerCorrect2(3,:), '--y', 'LineWidth', LINE_WIDTH);
...
legend('Use NALIF \sigma = 1,3','Use NALIF \sigma = 1.8','Use NALIF \sigma = 2.7', ...
    'Not Use \sigma = 1.3','Not Use \sigma = 1.8','Not Use \sigma = 2.7','Location', 'southeast')
...
print('-depsc', sprintf('%sFigure8.eps', figurePath)); % 保存为EPS格式
hold off;
